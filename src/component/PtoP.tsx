      ...
      <div className="w-full max-w-sm mt-20 flex flex-col">
        ...
        <div className="flex justify-between">
          <div className="flex gap-5">
            ...
            <Link to="/ptopsell">
              <h2 style={{color:"gray"}} className="font-bold">Sell</h2>
            </Link>
          </div>
        ...
        <div className="min-h-screen">

          {
            ads.ads.map((item, index)=>(
              <>
              {
                item.adType == "buy" && item.adStatus == "available"?
              ...
              //         <div>
              //           {/* <p className="text-sm" style={{color:"white"}}>N {item.tradeRate.toFixed(2)}</p>
              //           <p className="text-sm">Quantity {item.amount} {item.currencyCode}</p> */}
              ...
              //     </div>
              //     <div>
              //     <p className="text-sm" style={{color:"gray"}}><span className="text-xl" style={{fontWeight:"bold",color:"white"}}>{item.tradeRate.toFixed(2)}</span> {item.currencyCode}</p>
              //     <p className="text-sm" style={{paddingTop:"5px",color:"gray"}}><span className="text-xl" style={{fontWeight:"bold",color:"white"}}>{item.amount}</span> {item.assetToTrade}</p>
              //       {/* <h1>{item.amount} | {item.priceMargin}%</h1> */}
              //       <button onClick={()=>{
              //           setLoading(true);
      	      //           setTimeout(() => {
              //             setLoading(false); // Ensure loading state is reset after navigation
              //             navigate("/buydetails", {state:{id1:item._id,asset:item.assetToTrade}});
              //           }, 2000);
              //         }
              //       }
              //         //handleBuyClick
                  ...
                  <div style={{display:"flex",flexDirection:"column", alignItems:"end",justifyContent:"space-between",width:"50%"}}>
                    ...
                    <button onClick={()=>{
                        //setLoading(true);
      	               // setTimeout(() => {
                        //  setLoading(false); // Ensure loading state is reset after navigation
                          navigate("/buydetails", {state:{id1:item._id,asset:item.assetToTrade,adStatus:item.adStatus,userName:item.userName,min1:item.orderLimit.min,max1:item.orderLimit.max,am:item.amount}});
                       // }, 2000);
                      }
                     }
              //         //handleBuyClick
              ...
              :
              null
              // <div style={{width:"100%",paddingTop:"40px",paddingBottom:"40px",backgroundColor:"black"}}></div>
